...
import { useNavigation, useRefineContext, useResource, useTranslate, useUserFriendlyName } from '@refinedev/core';
...
export const ShowPage: FC<ShowProps> & {
  ...
  return (
    <>
      <PageHeader
        title={
          ...
          translate(
            ...
            `Show ${getUserFriendlyName(resource?.meta?.label ?? identifier, 'singular')}`
          )
        }
        ...
        extra={
          extra ?? (
            <div className="inline-flex items-center gap-x-2">
              {canEdit && <EditButton resource={resourceFromProps} />}
              {canDelete && (
                <DeleteButton
                  resource={resourceFromProps}
                  onSuccess={() => {
      ...
      />
      <div className="relative !mt-0 pt-4">{children}</div>
    </>
  );
};

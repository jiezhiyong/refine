import { FrontRouteModule, Prisma } from '@prisma/client';
...
import {
  ...
  TFrontRouteModuleDeleteRecord,
} from '~/routes/playground.frontRoute.frontRouteModule.edit.$id';
...
type TFrontRouteModule = FrontRouteModule & {
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const include: Prisma.FrontRouteModuleInclude = {
    operatedBy: { select: { name: true, avatar: true } },
    project: { select: { title: true } },
  };
  const args: Prisma.FrontRouteModuleFindManyArgs = {
    ...buildTableParams(tableParams),
    orderBy: { updatedAt: 'desc' },
    include,
  };
  const data = await dataService.findMany<TFrontRouteModule>('frontRouteModule', args, { request });

  return { initialData: data, include };
}

export default function FrontRouteModuleIndex() {
  const { initialData, include } = useLoaderData<typeof loader>();

  const tableRef = useRef<UseTableReturnType<BaseRecord, HttpError>>(null);
  const recordsRef = useRef<TFrontRouteModuleDeleteRecord[]>([]);
    ...
    <>
      ...
      >
        ...
        <TableEasy.Column
          ...
          enableHiding
          filter={(props: TableFilterProps) => <TableEasy.Filter.Search {...props} title="Search Project" />}
          cell={({ row: { original } }) => <Badge variant="outline">{original.project.title}</Badge>}
        />
            ...
            original.global ? (
              <Popover>
                ...
                <PopoverContent className="w-auto">
                  <pre className="text-sm">{JSON.stringify(JSON.parse(original.global), null, 2)}</pre>
                </PopoverContent>
              </Popover>
            ) : (
              '-'
        ...
        <TableEasy.Column
          ...
          accessorKey="id"
          id="actions"
          cell={({ row: { original } }: { row: { original: TFrontRouteModule } }) => (
            <>
              <ListButton
                ...
                icon={<Clock />}
                onClick={() => {
                  navigate(`./history/${original.id}?name=${original.project?.title}:${original.title}`);
                }}
              />
